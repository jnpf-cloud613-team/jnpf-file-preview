...
import cn.keking.utils.jwt.JwtUtil;
...
public class OnlinePreviewController {
    ...
    public String onlinePreview(String url, Model model, HttpServletRequest req) {
        ...
        //兼容DotNet
        if (StringUtil.isEmpty(realToken) && !StringUtil.isEmpty(token)) {
            if(!JwtUtil.getExp(token).after(new Date())){
                noToken = true;
            }
        }else {
            if (StringUtil.isEmpty(realToken)) {
                noToken = true;
            } else {
                token = token.replaceAll("bearer ", "");
                String redisCacheKey = "Authorization:login:token:" + token;
                if (!redisUtils.isExist(redisCacheKey) && !redisUtils.isExist(realToken)) {
                    noToken = true;
                }
            }
        }
        if (noToken){
            return otherFilePreview.notSupportedFile(model, "Token验证失败");
        }
        try {
